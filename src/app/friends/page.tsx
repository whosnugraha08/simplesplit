    ...
    <div className="content-padding pt-6 pb-4">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { setEditingFriend(null); setShowForm(true); }}
          className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold shadow-lg shadow-blue-500/20 active:scale-95 transition-all">
          + Tambah
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-border p-10 text-center">
          ...
          <button onClick={() => setShowForm(true)} className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-lg shadow-blue-500/20">
            + Tambah Teman
          </button>
        </div>
      ) : (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isMe && <span className="bg-blue-50 text-primary text-[10px] font-bold px-2 py-0.5 rounded-full">KAMU</span>}
                      {friend.is_admin && <span className="bg-amber-50 text-amber-700 text-[10px] font-bold px-2 py-0.5 rounded-full">ADMIN</span>}
                    </div>
                    {pmCount > 0 ? (
                      <p className="text-xs text-text-secondary mt-0.5">💳 {pmCount} metode pembayaran</p>
                    ) : (
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => { setEditingFriend(friend); setShowForm(true); }}
                      className="p-2 rounded-lg hover:bg-page transition-colors text-text-secondary" title="Edit">✏️</button>
                    {!isMe && (
                      <button onClick={() => setDeleteConfirm(friend.id)}
                        className="p-2 rounded-lg hover:bg-red-50 transition-colors text-text-secondary" title="Hapus">🗑️</button>
                    )}
                  </div>
                </div>
                {pmCount > 0 && (
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {friend.payment_methods!.map(pm => (
                      <span key={pm.id} className="bg-page text-text-secondary text-[10px] font-medium px-2 py-1 rounded-lg">{pm.label}</span>
                    ))}
                  </div>
                )}
              </div>
        ...
        <div className="fixed inset-0 overlay z-50 flex items-center justify-center p-4" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-white w-full max-w-lg rounded-3xl p-6 animate-slide-up" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 py-3 rounded-xl border border-border font-semibold text-sm">Batal</button>
  ...
  return (
    <div className="fixed inset-0 overlay z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white w-full max-w-lg rounded-3xl p-6 animate-slide-up max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        {error && <div className="bg-red-50 text-danger rounded-xl p-3 text-sm mb-4">⚠️ {error}</div>}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Contoh: Faiz"
              className="w-full px-4 py-3 rounded-xl border border-border bg-page text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition" required />
          </div>
          <div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button type="button" onClick={addPaymentMethod} className="text-sm text-primary font-semibold">+ Tambah</button>
            </div>
            {activePMs.length === 0 ? (
          ...
          <button type="submit" disabled={saving || !name.trim()}
            className="w-full py-3.5 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold text-sm disabled:opacity-50 transition-opacity active:scale-[0.98] shadow-lg shadow-blue-500/20">
