  ...
  if (!bill) {
    return (
      <div className="content-padding pt-6 text-center py-16">
        ...
        <p className="text-[var(--outline)]">Bill tidak ditemukan</p>
      </div>
    );
  }
  ...
  return (
    <div className="content-padding pt-6 pb-4">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => router.push('/bills')} className="text-xl text-[var(--outline)] p-1">←</button>
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[var(--outline)]">{formatDate(bill.bill_date)}</p>
        </div>
      </div>
      ...
      <div className="glass-card p-4 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-[var(--outline)]">Ditalangi oleh</p>
      ...
      {/* Items */}
      <div className="mb-4">
        <h2 className="text-sm font-semibold text-[var(--outline)] mb-2">Item ({items.length})</h2>
        <div className="glass-card divide-y divide-border">
          {items.map(item => (
            <div key={item.id} className="px-4 py-3 flex justify-between items-center">
              <div>
                <p className="text-sm font-medium">{item.item_name}</p>
                {item.quantity > 1 && <p className="text-xs text-[var(--outline)]">{item.quantity}x @ {formatRupiah(Number(item.item_price))}</p>}
              </div>
      ...
      {debts.length > 0 && (
        <div className="mb-4">
          <h2 className="text-sm font-semibold text-[var(--outline)] mb-2">Pembagian</h2>
                ...
                <div key={debt.id} className="glass-card p-4">
                  ...
                  {debt.notes && (
                    <>
                      ...
                      {isExpanded && (
                        <div className="mt-2 bg-[var(--surface-container)] rounded-lg p-2.5 animate-fade-in">
                          {debt.notes.split('\n').map((line, i) => (
                            <p key={i} className="text-[11px] text-[var(--outline)]">{line}</p>
                          ))}
                        </div>
                      )}
                    </>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 overlay z-50 flex items-center justify-center p-4" onClick={() => setShowDeleteConfirm(false)}>
          <div className="bg-[#1a1a2e] -lg border border-[var(--outline-variant)] w-full max-w-lg rounded-3xl p-6 animate-slide-up" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-white mb-2">Hapus Bill?</h3>
            <p className="text-sm text-[var(--outline)] mb-6">Bill &quot;{bill.title}&quot; dan semua data terkait akan dihapus.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-3 rounded-xl border border-[var(--outline-variant)] font-semibold text-sm text-white hover:bg-[var(--navy)]/5 transition-colors">Batal</button>
